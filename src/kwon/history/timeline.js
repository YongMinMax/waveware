...
export default function TimeLine () {
    ...
    return(
        <div>
            ...
            <div className={"flex "}>
                <div
                    className={"sticky top-[25vh] w-[400px] h-[250px] overflow-hidden"}>
                    {Object.keys(historyData).map((year) => (
                        <img
                            key={year}
                            className={`w-[400px] h-[250px] top-0 object-cover transition-transform duration-500 ease-in-out`}
                            src={historyData[year].image}
                            alt={`Image for ${year}`}
                            style={{
                                transform:
                                    selectedYear > year
